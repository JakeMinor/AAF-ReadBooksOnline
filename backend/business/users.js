...
module.exports = class userBusiness {
 async getAllUsers() {
  return userDataAccess.getAllAndPopulate({}, {path: 'roles', populate: {path: 'permissions', select: 'name'}}).then((users) => {
   return users.map(user => {return {id: user._id, username: user.username, email: user.email, roles: user.roles} })
 ...
 async createUser(newUser) {
  await validateNewUserData(newUser)
  const clientRoleId = (await rolesBusiness.getRoleByName("Client"))._id
  const user = {
   email: newUser.email,
   password: await hashPassword(newUser.password),
